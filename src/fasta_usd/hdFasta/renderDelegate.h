...
//
#ifndef PXR_IMAGING_PLUGIN_HD_Fasta_RENDER_DELEGATE_H
#define PXR_IMAGING_PLUGIN_HD_Fasta_RENDER_DELEGATE_H
...
#define HDFasta_RENDER_SETTINGS_TOKENS \
...
TF_DECLARE_PUBLIC_TOKENS(HdFastaRenderSettingsTokens, HDFasta_RENDER_SETTINGS_TOKENS);

///
/// \class HdFastaRenderDelegate
///
...
///
/// HdFasta Rprims create Fasta geometry objects in the render delegate's
/// top-level Fasta scene; and HdFasta's render pass draws by casting rays
...
/// CommitResources(). This can be used to commit GPU buffers or, in HdFasta's
/// case, to do a final build of the BVH.
///
class HdFastaRenderDelegate final : public HdRenderDelegate {
public:
    /// Render delegate constructor. This method creates the RTC device and
    /// scene, and links Fasta error handling to hydra error handling.
    HdFastaRenderDelegate();
    /// Render delegate constructor. This method creates the RTC device and
    /// scene, and links Fasta error ahndling to hydra error handling.
    /// It also populates initial render settings.
    HdFastaRenderDelegate(HdRenderSettingsMap const& settingsMap);
    /// Render delegate destructor. This method destroys the RTC device and
    /// scene.
    virtual ~HdFastaRenderDelegate();

    /// Return this delegate's render param.
    ///   \return A shared instance of HdFastaRenderParam.
    ...
    /// HdFastaRenderPass, which draw using Fasta's raycasting API.
    ...
    ///                     be drawn.
    ///   \return An Fasta renderpass object.
    ...
    ///   \return An Fasta instancer object.
    ...
    /// Fasta-specialized geometry containers like HdFastaMesh which map
    /// scene data to Fasta scene graph objects.
    ...
    ///   \param typeId The sprim type to create. This must be one of the types
    ///                 from GetSupportedSprimTypes().
    ///   \return An Fasta fallback sprim object.
    ...
    ///   \param typeId The bprim type to create. This must be one of the types
    ///                 from GetSupportedBprimTypes().
    ///   \return An Fasta fallback bprim object.
    ...
    /// Fasta doesn't currently use materials but raytraced backends generally
    /// specify "full".
...
private:
    ...
    HdFastaRenderDelegate(const HdFastaRenderDelegate &)             = delete;
    HdFastaRenderDelegate &operator =(const HdFastaRenderDelegate &) = delete;

    // Fasta initialization routine.
    void _Initialize();

    // Handle for an Fasta "device", or library state.
  //  RTCDevice _rtcDevice;

    // Handle for the top-level Fasta scene, mirroring the Hydra scene.
//    RTCScene _rtcScene;

    // A version counter for edits to _scene.
    std::atomic<int> _sceneVersion;

    // A shared HdFastaRenderParam object that stores top-level Fasta state;
    // passed to prims during Sync().
    std::shared_ptr<HdFastaRenderParam> _renderParam;
    ...
    // An Fasta renderer object, to perform the actual raytracing.
    FstRenderer _renderer;

    // A list of render setting exports.
    HdRenderSettingDescriptorList _settingDescriptors;

    // A callback that interprets Fasta error codes and injects them into
    // the hydra logging system.
    static void HandleFastaError(const int code, const char *msg);
};


PXR_NAMESPACE_CLOSE_SCOPE

#endif // PXR_IMAGING_PLUGIN_HD_Fasta_RENDER_DELEGATE_H
